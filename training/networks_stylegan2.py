...
class SynthesisLayer(torch.nn.Module):
    ...
    def __init__(
            ...
            resample_filter=[
                1, 3, 3, 1
            ],  # Low-pass filter to apply when resampling activations.  # noqa
            conv_clamp=None,  # Clamp the output of convolution layers to +-X, None = disable clamping.  # noqa
    ...
    def extra_repr(self):
        return ' '.join([
            f'in_channels={self.in_channels:d}, out_channels={self.out_channels:d}, w_dim={self.w_dim:d},',  # noqa
            f'resolution={self.resolution:d}, up={self.up}, activation={self.activation:s}'  # noqa
        ])
...
class ToRGBLayer(torch.nn.Module):
    ...
    def extra_repr(self):
        return (f'in_channels={self.in_channels:d}, '
                f'out_channels={self.out_channels:d}, w_dim={self.w_dim:d}')
...
class SynthesisBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        conv_clamp=256,  # Clamp the output of convolution layers to +-X, None = disable clamping.  # noqa
        use_fp16=False,  # Use FP16 for this block?
        fp16_channels_last=False,  # Use channels-last memory format with FP16?
        fused_modconv_default=True,  # Default value of fused_modconv. 'inference_only' = True for inference, False for training.  # noqa
    ...
    def forward(self,
        ...
        memory_format = torch.channels_last if (
            self.channels_last and not force_fp32) else torch.contiguous_format
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def __init__(
            ...
            channel_base=32768,  # Overall multiplier for the number of channels.  # noqa
...
class NeRFSynthesisNetwork(torch.nn.Module):
    ...
    def __init__(
            ...
            img_channels,  # Number of color channels.
            channel_base=32768,  # Overall multiplier for the number of channels.  # noqa
            channel_max=512,  # Maximum number of channels in any layer.  # noqa
            num_fp16_res=4,  # Use FP16 for the N highest resolutions.  # noqa
            nerf_kwargs={},
            bbox_kwargs={},
            decoder_kwargs={},
            bg_decoder_kwargs={},
        ...
        if 'resolution_vol' in nerf_kwargs:
            nerf_res = nerf_kwargs['resolution_vol']
            nerf_res_log2 = int(np.log2(nerf_kwargs['resolution_vol']))
            assert 2**nerf_res_log2 == nerf_res
        else:
            # use nerf to instead the const input
            nerf_res = self.block_resolutions[0]
            nerf_rgb_dim = channels_dict[nerf_res]
        ...
        for res in self.block_resolutions:
            if res < nerf_res:
                ...
                nerf_rgb_dim = channels_dict[res]
                continue

            self.syn_res.append(res)
            in_channels = channels_dict[res // 2] if res > 4 else 0
            out_channels = channels_dict[res]
            use_fp16 = (res >= fp16_resolution)
            is_last = (res == self.img_resolution)
        ...
        decoder_kwargs['rgb_out_dim'] = nerf_rgb_dim
        bg_decoder_kwargs['rgb_out_dim'] = nerf_rgb_dim
        self.nerf = self.build_nerf(decoder_kwargs, bg_decoder_kwargs,
                                    bbox_kwargs, nerf_kwargs,
                                    nerf_resume_kwargs)
        self.nerf_res = nerf_res

    def build_nerf(self, decoder_kwargs, bg_decoder_kwargs, bbox_kwargs,
                   nerf_kwargs, resume_kwargs):
